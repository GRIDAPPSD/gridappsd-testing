...
if HAS_DOCKER:
    ...
    GRIDAPPSD_CONF_DIR = Path(__file__).resolve().parent.joinpath("./gridappsd/conf")
    print(GRIDAPPSD_CONF_DIR)
    ...
    def run_dependency_containers(stop_after=False) -> Containers:
        ...
        print(DEFAULT_DOCKER_DEPENDENCY_CONFIG)
        ...
        try:
            ...
            message_str = "An error occurred while trying to start the containers " + str(e)
            _log.error(message_str)
        finally:
            if stop_after:
                containers.stop()
    ...
    def run_gridappsd_container(stop_after=True, rebuild_if_present=False) -> Container:
        ...
        if parent_container:
            ...
            env[GRIDAPPSD_ENV_ENUM.GRIDAPPSD_USER.name] = 'system'
            env[GRIDAPPSD_ENV_ENUM.GRIDAPPSD_PASSWORD.name] = 'manager'
        ...
        print('BEORE CONTAINERS')
        print(DEFAULT_GRIDAPPSD_DOCKER_CONFIG)
        ...
        try:
            ...
            print('GRIDAPPSD CONTAINER NOT FOUND')
        ...
        try:
            if gridappsd_container is None:
                ...
                while True:
                    ...
                    try:
                        g = GridAPPSD(username='system', password='manager')
                        print('GRIDAPPSD ')
                        print(g)
                        if g.connected:
                            print('IS CONNECTED')
            ...
            message_str = "An error occurred while trying to start the gridappsd container " + str(e)
            _log.error(message_str)      
            print(e)      
            print('STACK TRACE')
            print(traceback.format_exc())
        finally:
            print('ABOUT TO STOP CONTAINER')
            if stop_after:
                containers.stop()
